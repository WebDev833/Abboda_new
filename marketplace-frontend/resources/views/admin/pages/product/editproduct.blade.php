...
<!-- End Page Header -->
{!! Form::model($product, ['route'=>['admin.editproductsave',$product->id],'data-parsley-validate'=>'']) !!}
@method('PATCH')
<div class="row">
...
@if (old('company_id') || (Route::current()->getName() == 'admin.editproduct'))
@php
    $com_id = (old('company_id')) ? old('company_id') : $product->company_id;
    $cat_id = (old('category_id')) ? old('category_id') : $product->category_id;
@endphp
<script>
  $(document).ready(function(){
      var cval = "{{$com_id}}";
